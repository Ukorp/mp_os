    ...
    std::vector<typename AVL_tree<tkey, tvalue>::iterator_data> &expected_result)
{
    ...
    {
        std::cout << "DEPTH: " << (*it)->depth << ", KEY: " << ((*it)->key) << ", VALUE: " << ((*it)->value) << ", SUB_HEIGHT: " << reinterpret_cast<typename AVL_tree<tkey, tvalue>::iterator_data const *>(*it)->subtree_height << std::endl;
        std::cout << "DEPTH: " << item.depth << ", KEY: " << (item.key) << ", VALUE: " << (item.value) << ", SUB_HEIGHT: " << item.subtree_height << std::endl;
...
TEST(AVLTreePositiveTests, test2)
{
    ...
    search_tree<int, int> *avl = new AVL_tree<int, int>(nullptr, nullptr, key_comparer());
...
TEST(AVLTreePositiveTests, test3)
{
    ...
    search_tree<std::string, int> *avl = new AVL_tree<std::string, int>(nullptr, logger, key_comparer());
...
TEST(AVLTreePositiveTests, test4)
{
    ...
    search_tree<int, std::string> *avl1 = new AVL_tree<int, std::string>(nullptr, logger, key_comparer());
